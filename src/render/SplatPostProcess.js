 ...
 * SplatPostProcess
 *
 * Post-processing pipeline for Gaussian splat renderers. Captures the splat
 * output to an offscreen FBO, then composites to screen with:
 *
 *  1. Sobel edge detection on luminance → silhouette edges
 *  2. VIB3-style procedural inscription masked to edges
 *  3. HDR tonemap (Reinhard + gamma)
 *
 * Designed to be compatible with the HybridRenderPipeline's layer compositor.
 * The detected edges and inscription can be exported as separate textures for
 * use as inputs to EdgeInscriptionLayer or the 4-layer HybridRenderPipeline.
 *
 * Usage:
 *   const pp = new SplatPostProcess(gl, { edgeColor: [0.3, 0.7, 1.0] });
 *   pp.beginCapture();          // bind offscreen FBO
 *   renderer.render(...);       // splats go to FBO
 *   pp.endCaptureAndComposite(time);  // edge detect + tonemap to screen
 */
...
/* ------------------------------------------------------------------ */
/*  Composite fragment: edge detection + inscription + tonemap         */
/* ------------------------------------------------------------------ */
...
uniform float u_inscGeometry;     // 0-23 VIB3 pattern variant
...
/* ---- Luminance helper -------------------------------------------- */
float lum(vec2 uv) {
    vec3 c = texture(u_splatColor, uv).rgb;
...
/* ---- VIB3 procedural patterns (8 base geometries) ---------------- */
...
void main() {
    ...
    float alpha = texture(u_splatColor, v_uv).a;

    if (u_enableEdges > 0.5) {
        vec2 ts = u_texelSize;

        // ---- Sobel edge detection on luminance ----------------------
        float d00 = lum(v_uv + vec2(-ts.x, -ts.y));
        float d10 = lum(v_uv + vec2(  0.0, -ts.y));
        float d20 = lum(v_uv + vec2( ts.x, -ts.y));
        float d01 = lum(v_uv + vec2(-ts.x,   0.0));
        float d21 = lum(v_uv + vec2( ts.x,   0.0));
        float d02 = lum(v_uv + vec2(-ts.x,  ts.y));
        float d12 = lum(v_uv + vec2(  0.0,  ts.y));
        float d22 = lum(v_uv + vec2( ts.x,  ts.y));
        ...
        if (u_enableInscription > 0.5 && edge > 0.01) {
            // ---- VIB3 procedural inscription on edges ---------------
            ...
            pat = 0.6 + 0.4 * pat; // floor so edges always show
            ...
            // Simple edge glow without inscription
...
export class SplatPostProcess {
     ...
     * @param {number[]} [options.edgeColor]       RGB [0-1] edge glow color
     * @param {number}   [options.edgeIntensity]   Edge brightness multiplier
     * @param {number}   [options.edgeSensitivity] Sobel magnitude scale
     * @param {boolean}  [options.enableEdges]      Toggle edge detection
     * @param {boolean}  [options.enableInscription] Toggle VIB3 inscription on edges
     * @param {number}   [options.inscGeometry]    VIB3 geometry variant 0-23
     * @param {number}   [options.inscScale]       Inscription pattern scale
     * @param {number}   [options.inscSpeed]       Inscription animation speed
     * @param {boolean}  [options.enableTonemap]   Toggle HDR tonemap
     * @param {number}   [options.exposure]        Tonemap exposure
     * @param {number}   [options.gamma]           Tonemap gamma
     */
    constructor(gl, options = {}) {
        ...
        this.inscGeometry = options.inscGeometry ?? 3;  // torus pattern
    ...
    /** Returns the captured color texture (after endCaptureAndComposite). */
    get colorTexture() { return this._fbo ? this._fbo.colorTexture : null; }

    /**
     * Bind the offscreen FBO. Call this before your splat renderer's render().
     * The renderer will draw into the FBO instead of the screen.
     */
    beginCapture() {
        const gl = this.gl;
    ...
    /**
     * Unbind the FBO, run edge detection + inscription + tonemap, and
     * composite to the default framebuffer (screen).
     *
